          ...
          EngineConstants::DebugChannelFragmentShader))
{
    ...
    // Staged rollout: validate direct viewport rendering before re-enabling the full post stack.
    m_enabled = false;
    m_ssaoEnabled = false;
    ...
    const DirectionalShadowSettings& shadowSettings) const
{
    ...
    if (m_sceneFramebuffer->HasSplitLighting())
    {
        ...
        m_compositeShader->BindTextureSlot(2, m_ssaoBlurTarget.srvCpuHandle);
        m_compositeShader->BindTextureSlot(3, m_sceneFramebuffer->GetDepthSrvCpuHandle());
    ...
    else if (runSsao)
    {
        ...
        m_compositeShader->SetInt(
            ...
            m_debugMode == RenderDebugMode::CompositeOcclusion ? 1 : 0);
        m_compositeShader->BindTextureSlot(0, m_sceneFramebuffer->GetColorSrvCpuHandle(0));
        m_compositeShader->BindTextureSlot(1, m_ssaoBlurTarget.srvCpuHandle);
