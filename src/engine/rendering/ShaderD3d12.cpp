...
namespace
{
    ...
    ComPtr<ID3DBlob> CompileHlsl(const std::string& source, const char* entry, const char* target)
    {
        struct IncludeHandler final : ID3DInclude
        {
                ...
                UINT* bytes) override
            {
                try
                {
                    const std::string path = std::string("assets/shaders/") + fileName;
        ...
        if (FAILED(hr))
        {
            std::string message = "Shader compile failed (";
            message += target;
            message += ")";
            if (errors != nullptr)
            {
                const char* errorText = static_cast<const char*>(errors->GetBufferPointer());
                const std::size_t errorSize = errors->GetBufferSize();
                if (errorText != nullptr && errorSize > 0)
                {
                    message.append(": ");
                    message.append(errorText, errorSize);
                }
            }
...
{
    BuildFromHlsl(ShaderPath::ToHlslPath(vertexPath), ShaderPath::ToHlslPath(fragmentPath));
}
...
Shader::~Shader()
{
    ...
    {
        static_cast<ID3DBlob*>(m_vertexShader)->Release();
    }

    if (m_pixelShader != nullptr)
    {
        static_cast<ID3DBlob*>(m_pixelShader)->Release();
    }
}
...
void Shader::BuildFromHlsl(const std::string& vertexPath, const std::string& fragmentPath)
{
    ...
    ComPtr<ID3DBlob> vertexBlob = CompileHlsl(vertexSource, "main", "vs_5_0");
    ComPtr<ID3DBlob> pixelBlob = CompileHlsl(fragmentSource, "main", "ps_5_0");
    m_vertexShader = vertexBlob.Detach();
    m_pixelShader = pixelBlob.Detach();
    ...
    ThrowIfFailed(
        D3DReflect(
            static_cast<ID3DBlob*>(m_vertexShader)->GetBufferPointer(),
            static_cast<ID3DBlob*>(m_vertexShader)->GetBufferSize(),
            IID_PPV_ARGS(&vsReflection)),
        "VS reflection failed");
    ThrowIfFailed(
        D3DReflect(
            static_cast<ID3DBlob*>(m_pixelShader)->GetBufferPointer(),
            static_cast<ID3DBlob*>(m_pixelShader)->GetBufferSize(),
            IID_PPV_ARGS(&psReflection)),
        "PS reflection failed");
    ...
    psoDesc.VS = {static_cast<ID3DBlob*>(m_vertexShader)->GetBufferPointer(),
        static_cast<ID3DBlob*>(m_vertexShader)->GetBufferSize()};
    psoDesc.PS = {static_cast<ID3DBlob*>(m_pixelShader)->GetBufferPointer(),
        static_cast<ID3DBlob*>(m_pixelShader)->GetBufferSize()};
