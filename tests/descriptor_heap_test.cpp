...
namespace
{
    void RunFixedDescriptorHeapTests()
    {
        ...
        test::ExpectTrue(
            heap.IsAllocated(2) && heap.IsAllocated(3) && heap.IsAllocated(4),
            "Block slots should be marked used");
        ...
        test::ExpectTrue(
            largeBlock != FixedDescriptorHeap::kInvalid,
            "Large block should fit after freeing indices 2-4");
        ...
        test::ExpectTrue(
            heap.AllocateOne() == FixedDescriptorHeap::kInvalid,
            "Heap should return invalid when full");
        test::ExpectTrue(
            !heap.IsValidIndex(FixedDescriptorHeap::kInvalid),
            "Invalid index must not be passed to GPU handle math");
        ...
        test::ExpectTrue(
            heap.UsedCount() == usedBeforeInvalidFree,
            "Freeing invalid indices should be a no-op");

        FixedDescriptorHeap tiny(2);
        test::ExpectTrue(
            tiny.AllocateBlock(3) == FixedDescriptorHeap::kInvalid,
            "Oversized block request should fail");
        test::ExpectTrue(
            tiny.AllocateOne() == 0 && tiny.AllocateOne() == 1,
            "Tiny heap should allocate two slots");
        test::ExpectTrue(
            tiny.AllocateOne() == FixedDescriptorHeap::kInvalid,
            "Tiny heap should exhaust after two slots");
...
int main()
{
    test::ResetFailures();
    test::ResetTestRun();
    test::RunTest("fixed_descriptor_heap", RunFixedDescriptorHeapTests);
    test::PrintSummary();
