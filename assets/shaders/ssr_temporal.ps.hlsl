// SSR temporal — motion-vector reprojection with luminance agreement and receiver-edge guard.
// Reduces edge shimmer without blanket blur on stable interior reflection texels.
...
{
    const float currentLuma = TraceLuma(current) * current.a;
    const float historyLuma = TraceLuma(history) * history.a;
